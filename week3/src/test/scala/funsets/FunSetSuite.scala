...
class FunSetSuite extends FunSuite {
	...
	trait TestSets {
		...
		val s5: Set = singletonSet(4)
		val setPositiveNumbers: Set = union(singletonSet(1), singletonSet(300))
		val setNegativeNumbers: Set = union(singletonSet(-10), singletonSet(-99))
		val setPositiveAndNegativeNumbers: Set = union(setPositiveNumbers, setNegativeNumbers)
		val setEvenNumbers: Set = union(singletonSet(4), singletonSet(6))
		val setOddNumbers: Set = union(singletonSet(3), singletonSet(9))
		val setEvenAndOddNumbers: Set = union(setEvenNumbers, setOddNumbers)
	}
	...
	test("filter returns the subset of one set for which a parameter function holds") {
		new TestSets {
			val filterSet1: Set = filter(s1, { elem: Int => elem < 2 })
			assert(contains(filterSet1, 1))
			val filterSet2: Set = filter(s3, { elem: Int => elem > 5 })
	...
	test("map function") {
		new TestSets {
			val mapEvenSetToOdd: Set = map(setEvenNumbers, { elem: Int => elem + 1 })
			printSet(mapEvenSetToOdd)
			assert(contains(mapEvenSetToOdd, 5) && contains(mapEvenSetToOdd, 7))
			val mapOddSetToEven: Set = map(setOddNumbers, { elem: Int => elem * 2 })
